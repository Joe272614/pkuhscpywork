...
#价格与建造年份的关系分析
year_pricere = pd.concat([data_train['SalePrice'], data_train['YearBuilt']], axis=1)
plt.figure(figsize = (20, 18))
sns.boxplot(x = 'YearBuilt', y = 'SalePrice', data = year_pricere)

#价格与售出年份的关系
Yrsold_price = pd.concat([data_train['YrSold'],data_train['SalePrice']], axis = 1)
plt.figure(figsize = (30, 27))
Yrsold_price.plot.scatter(x='YrSold',y = 'SalePrice', s = 4, color = 'blue')

# 绘制和价格相关的特征的散点图
TotalBsmtSF_SalePrice = pd.concat([data_train['SalePrice'],data_train['TotalBsmtSF']],axis=1)
...
data_test.shape
#测试数据缺失值统计
data_test.isnull().sum().sort_values(ascending = False).head(10000)
